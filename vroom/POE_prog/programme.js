...
let modules = [
	...
	{name:"HTML 5 + CSS3",										duration:2,	coach:"Sebastien P", start:"2017-12-20"},
	{name:"Javascript/ Angular",								duration:5,	coach:"Jean Frédéric V + Cyril M", start:"2017-12-22"},
	...
	{name:"Ecrits professionnels et atelier de candidature",	duration:2,	coach:"", start:"2018-01-12"},
	{name:"Java - Les fondamentaux et le développement JavaSE",	duration:5,	coach:"", start:"2018-01-16"},
	{name:"JPA - Persistence des données (Hibernate)",			duration:3,	coach:"", start:"2018-01-23"},
	{name:"Java - Programmation client",						duration:3,	coach:"", start:"2018-01-26"},
	...
	{name:"Travaux pratiques et passage de la certification \"M2i langage de développement orientée Java\"",	duration:2,	start:"2018-03-06"}
];
...
for (let i = 0; i < modules.length; i++)
{
	...
	celulle_nom.innerHTML        = modules[i].name;
	celulle_duree_j.innerHTML    = modules[i].duration;
	celulle_duree_h.innerHTML    = modules[i].duration * 7 ;
	celulle_coach.innerHTML      = modules[i].coach;

	let date_debut = new Date(modules[i].start);
	let date_fin = new Date();
	date_fin.setTime(findLastDate(date_debut, modules[i].duration));

	celulle_date_debut.innerHTML = jour_en_fr(date_debut.getDay()) + " " + date_debut.toLocaleDateString();
	celulle_date_fin.innerHTML   = jour_en_fr(date_fin.getDay()) + " " + date_fin.toLocaleDateString();
...
document.getElementById('programme_footer_duree_j').innerHTML = duree_j;
document.getElementById('programme_footer_duree_h').innerHTML = duree_h;